...
- **Функционал**:
    - Просмотр всех заказов с пагинацией (15 на страницу)
...
- **Функционал**:
    ...
    - Валидация на фронтенде (обязательные поля, qty >= 1)
...
- **Функционал**:
    ...
    - Отображение `supplier_ref` (если есть)
    - Отображение `supplier_check_attempts` (количество попыток проверки поставки)
- **Детали**:
...
- **Действия**:
    - Клик на SKU → переход к истории движений
- **Детали**:
    - Отображается колонка "Всего" (сумма доступного и зарезервированного количества)
    - Кликабельные заголовки колонок для сортировки с индикаторами направления (↑/↓)
